...
- 显示套餐名与续订日期（接口不返回，需自行配置常量）
- 每 15 分钟自动刷新，刷新时刻对齐整刻（:00 / :15 / :30 / :45）
- 双击窗口立即刷新；右键菜单：置顶 / 立即刷新 / 白天黑夜切换 / 退出
- 右键可分别勾选显示 Kimi / Codex 卡片，选择写入 `settings.json`，重启后自动沿用
- **黑夜 / 白天双主题**
...
打开 `quota_monitor.py`，文件顶部的「个人配置」区块有几个常量需要按自己的情况修改。右键菜单里 Kimi/Codex 的显示开关保存在同目录 `settings.json`，无需手动维护：
